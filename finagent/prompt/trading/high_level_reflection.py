...
class HighLevelReflectionTrading(Prompt):
    ...
    def run(self,
        ...
        if call_provider:
            ...
            #                                        model=self.model,
            #                                        messages=message)
            REQUIRED_KEYS = ["reasoning", "improvement", "summary", "query"]
            max_retries = 5
            for attempt in range(max_retries):
                print(f"[Attempt {attempt + 1}/{max_retries}] Calling LLM provider...")
                response_dict, res_html = self.get_response_dict(provider=provider, model=self.model, messages=message)
                try:
                    reasoning = response_dict["reasoning"]
                    improvement = response_dict["improvement"]
                    summary = response_dict["summary"]
                    ...
                    if attempt < max_retries - 1:
                        ...
                        print("[ERROR] Max retries reached. Filling missing fields with default values.")
                        reasoning = response_dict.get("reasoning", "[MISSING REASONING]")
                        improvement = response_dict.get("improvement", "[MISSING IMPROVEMENT]")
